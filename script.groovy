...
def pushImage() {
    ...
    withCredentials([usernamePassword(
        ...
        sh "echo \${DOCKER_REGISTRY_PASSWORD} | docker login -u \${DOCKER_REGISTRY_USERNAME} --password-stdin"
        sh "docker build -t docker.io/${env.DOCKER_REGISTRY}:Build-${COMMIT_HASH}-APP ."
        sh "docker push docker.io/${env.DOCKER_REGISTRY}:Build-${COMMIT_HASH}-APP"
    }
    return COMMIT_HASH
}
...
def deploy() {
    ...
    withCredentials([[
        ...
        // Replace placeholders in deployment.yml with actual values and apply it
        sh '''
        sed -i "s|\\$\\{DOCKER_REGISTRY\\}|${DOCKER_REGISTRY}|g" ./kubernetes-manifests/deployment.yml
        sed -i "s|\\$\\{COMMIT_HASH\\}|${COMMIT_HASH}|g" ./kubernetes-manifests/deployment.yml
        '''
